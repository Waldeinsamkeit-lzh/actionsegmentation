import sys
import os
sys.path.append(os.path.abspath(os.path.join(os.path.dirname(__file__), "segment")))
...
app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"],  # 你也可以写 ["http://localhost:3000"]
...
async def upload_video(video: UploadFile = File(...)):
    ...
    mapping_path=r"C:\Users\Administrator\actionsegmentation\backend\map\mapping.txt"
    # video_file_path = os.path.join('uploads', video_filename)
    video_file_path = os.path.join(r'C:\Users\Administrator\actionsegmentation\savedirgpu21withspace33', video_filename)
    ...
    # 加载模型权重
    with gzip.open(r'C:\Users\Administrator\actionsegmentation\backend\segment\checkpoints\best_ckpt.gz', 'rb') as f:
        best_ckpt = pickle.load(f)
    iteration = best_ckpt.iteration
    model_file = rf'C:\Users\Administrator\actionsegmentation\backend\segment\checkpoints\ckpts\network.iter-{iteration}.net'
    ...
    try:
        ...
        id2label = load_mapping(mapping_path)
        label_pred = [id2label.get(p, f"未知({p})") for p in pred]
        return {
        "predictions": pred,         # 原始数字
        "label_predictions": label_pred  # 转换后的中文标签
        }
