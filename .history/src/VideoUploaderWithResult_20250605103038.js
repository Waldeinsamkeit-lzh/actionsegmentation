...
export default function VideoUploaderWithResult() {
  ...
  const uniqueLabels = [...new Map(segments.map(s => [s.label, s.labelText]))];
  const categoryFrameCounts = {};
  segments.forEach(seg => {
    categoryFrameCounts[seg.label] = (categoryFrameCounts[seg.label] || 0) + (seg.end - seg.start + 1);
  });
  ...
  useEffect(() => {
    if (segments.length > 0 && canvasRef.current) {
      ...
      labels.forEach((label, index) => {
        const height = (frameCounts[index] / maxFrames) * (chartHeight - 40);
        ctx.fillStyle = getColor(label);
        ctx.fillRect(index * (canvas.width / labels.length), chartHeight - height, barWidth, height);
        ctx.fillStyle = "#000";
        ctx.font = "12px Arial";
        ctx.textAlign = "center";
        ctx.fillText(frameCounts[index], index * (canvas.width / labels.length) + barWidth / 2, chartHeight - height - 5);
        ctx.fillText(`类 ${label}`, index * (canvas.width / labels.length) + barWidth / 2, chartHeight - 15);
      });
    }
  }, [segments]);
